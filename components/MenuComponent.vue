        ...
        <NuxtLink :to="`/item/${menu.id}`">
            ...
            <div id="MenuDetails">
                <span class="flex items-center justify-start gap-3 px-1 pt-1">
                    <span class="text-[#FF6674] font-semibold">SR {{ priceComputed }}</span>
                    <span class="text-gray-500 text-sm text-light line-through">SR {{ oldPriceComputed }}</span>
                </span>
...
const priceComputed = computed (() => {
    return menu.value.price / 100
})

const oldPriceComputed = computed (() => {
    let res = (menu.value.price + (menu.value.price / 20)) /100
